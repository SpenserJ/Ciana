...
var Screen = module.exports.Screen = function Screen(ciana, name) {
  ...
  for (i = 0; i < panel_list.length; i++) {
    ...
    var funcs = {}, templateKeys = Object.keys(provider.toTemplate);
    for (i2 = 0; i2 < templateKeys.length; i2++) {
      funcs[templateKeys[i2]] = provider.toTemplate[templateKeys[i2]].toString();
    }
    self.provider_templates[provider.name] = funcs;
